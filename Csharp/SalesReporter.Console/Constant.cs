namespace SalesReporterKata;

public class Constant
{
    public static string SALES_VIEWER_TITLE = "=== Sales Viewer ===";
    public static string NUMBER_OF_SALES = " Number of sales";
    public static string NUMBER_OF_CLIENTS = " Number of clients";
    public static string TOTAL_ITEMS_SOLD = " Total items sold";
    public static string TOTAL_SALES_AMOUNT = " Total sales amount";
    public static string AVERAGE_AMOUNT_SALE = " Average amount/sale";
    public static string AVERAGE_ITEM_PRICE = " Average item price";
}